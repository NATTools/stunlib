...
CTEST (realworld, integrity_turn_response)
{
  ...
  ASSERT_TRUE( stunlib_DecodeMessage(turnResp, numbytes, &stunResponse, NULL, NULL));

  stunlib_createMD5Key((unsigned char *)md5, user, realm, password);

  ASSERT_TRUE( stunlib_checkIntegrity(turnResp, numbytes, &stunResponse, (unsigned char *)md5, keyLen));
}
